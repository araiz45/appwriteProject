...
function SearchPage() {
  ...
  const [searchStringForm, setSearchStringForm] = useState("");
  ...
  const handleSearch = (ev: React.FormEvent<HTMLFormElement>) => {
    ev.preventDefault();
    const arrayOfString = searchStringForm.toLowerCase().split(" ");
      ...
      <section className="mt-10 px-5">
        <div className="">
          <div className="font-bold text-2xl">
            <span className="">Your search query is </span>
            <span className="bg-primary text-background py-1 px-2 rounded-md">
              &quot;
              {searchString &&
                searchString[0].toUpperCase() +
        ...
        <div className="bg-secondary h-20 mt-5 flex justify-center items-center rounded-lg">
          ...
          >
            <Input
              type="text"
              onChange={(e) => setSearchStringForm(e.target.value)}
              value={searchStringForm}
